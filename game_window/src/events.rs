...
pub(crate) fn convert_keycode(input: VirtualKeyCode) -> KeyCode {
    macro_rules! convert_keycode_impl {
        ($input:expr, $($key:ident),*,) => {
            match $input {
                $(
                   VirtualKeyCode::$key => KeyCode::$key,
                )*
            }
        };
    }

    convert_keycode_impl!(
        input,
        Key1,
        Key2,
        Key3,
